...
public class BehaviorMecanim : MonoBehaviour
{
	...
	public Node Node_TurnBack(Val<Vector3> targ, Vector3 curPos){

		return new LeafInvoke(
			
			() => this.Character.NavTurnBack(targ, curPos),
			() => this.Character.NavOrientBehavior(
			OrientationBehavior.LookForward));
	}
	

	//go to sit on something
	public Node Node_Sit() {

		return new LeafInvoke (() => this.Character.SitDown ());
	}

	public Node Node_Stand() {
		return new LeafInvoke (() => this.Character.StandUp ());
	}

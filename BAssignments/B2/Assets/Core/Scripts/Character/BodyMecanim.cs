...
public class BodyMecanim : MonoBehaviour
{
    ...
    public void HandAnimation(string gestureName, bool isActive)
    {
        ...
        switch (gestureName.ToUpper())
        {
			...
			case "CHEER":
				if (isActive)
					this.animator.SetTrigger("H_Cheer");
				break;
            case "YAWN":
                if (isActive)
	...
	public void BodyAnimation(string gestureName, bool isActive)
	{
		...
		case "PUNCH":
			if (isActive)
				this.animator.SetTrigger("B_Punch");
			break;
		case "WAVEHANDS":
			if (isActive)
				this.animator.SetTrigger("B_WaveHands");
			break;
		case "SWEEPLEG":
			if (isActive)
				this.animator.SetTrigger("B_SweepLeg");
			break;
		case "FLYKICK":
			if (isActive)
				this.animator.SetTrigger("B_FlyKick");
			break;
        case "DUCK":
            this.animator.SetBool("B_Duck", isActive);
            break;
		case "THROW":
			this.animator.SetBool("B_ThrowBall", isActive);
			break;
		}
	}
